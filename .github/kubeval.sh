...
CHART_DIRS="$(git diff --find-renames --name-only "$(git rev-parse --abbrev-ref HEAD)" remotes/origin/main -- helm | cut -d '/' -f 2 | uniq)"
...
for CHART_DIR in ${CHART_DIRS}; do
  (cd helm/${CHART_DIR}; helm dependency build)
  helm template \
    $apis \
    helm/"${CHART_DIR}" | kubeval \
      --strict \
